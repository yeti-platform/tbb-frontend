<template>
  <div>
    <div v-bind:key="killChain.id" v-for="killChain in killChains" class="killchain mt-4 mb-4">
      <h3>{{ killChain.human_name }}</h3>
      Slug: <code>{{ killChain.name }}</code>
      ...
      <div>
        ...
        <div class="form-row show-on-hover" v-for="phase in killChain.settings[killChain.name]" v-bind:key="phase.name">
          ...
          <div class="form-group">
            <a
              href="#"
              class="btn btn-outline-danger show-on-hover"
              @click="removeField(killChain.settings[killChain.name], phase)"
            >
          ...
          <div class="form-row">
            <div class="col">
              <button
                ...
                @click="addField(killChain.settings[killChain.name])"
              >
                Add phase
              </button>
              <button
                ...
                @click="updateKillChain(killChain, killChain.settings[killChain.name])"
              >
...
export default {
  ...
  methods: {
    ...
    updateKillChain(killChain, phases) {
      this.saving = true;
      axios
        .put("/settings/killchains/" + killChain.name + "/", { phases: phases })
        .then(response => {
