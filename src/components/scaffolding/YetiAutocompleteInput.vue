<template>
  <div>
    ...
    />
    <small v-if="autocompleteUrl" class="form-text text-muted"
      >Autocompleting from <code>{{ autocompleteUrl }}</code></small
    >
  </div>
</template>
  ...
  computed: {
    filteredItems() {
      return this.autocompleteItems
        .filter(item => new RegExp(this.item, "i").test(item.name))
        .map(item => Object({ text: item.name }));
    }
  },
  mounted() {
